...
curl -H "Authorization: token ${GITHUB_TOKEN}" \
  ...
  -L https://api.github.com/repos/OpenDRR/model-inputs/contents/exposure/general-building-stock/BldgExp_CA_v2p5p3_master.csv
...
#  -L https://api.github.com/repos/OpenDRR/model-inputs/contents/exposure/building-inventory/SiteExp_MetroVan_v4_master.csv

#DOWNLOAD_URL=`grep -o '"download_url": *.*' SiteExp_MetroVan_v4_master.csv | cut -f2- -d: | tr -d '"'| tr -d ',' `
#curl -o SiteExp_MetroVan_v4_master.csv \
#  -L $DOWNLOAD_URL
#psql -h db-opendrr -U ${POSTGRES_USER} -d ${DB_NAME} -a -f Create_table_canada_site_exposure.sql
#psql -h db-opendrr -U ${POSTGRES_USER} -d ${DB_NAME} -a -f Create_table_canada_site_exposure.sql
...
#psql -h db-opendrr -U ${POSTGRES_USER} -d ${DB_NAME} -a -f Create_physical_exposure_site_level_indicators_PhysicalExposure.sql
